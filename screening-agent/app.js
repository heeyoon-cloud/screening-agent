...
function getApiKey() {
  return localStorage.getItem(API_KEY_SK) || '';
}

function saveApiKey(key) {
  localStorage.setItem(API_KEY_SK, key);
}
...
async function runScreening() {
  ...
  try {
    ...
    lastResult = {
      ...
      totalTeams: document.getElementById('total-n').value||'?',
      selectGoal: document.getElementById('select-n').value||'?',
      stage: document.getElementById('stage').value||'미지정',
      result: full, notionSaved: false
    };
    S.history.unshift(lastResult); saveState(); renderHistory();
    document.getElementById('notion-btn').disabled = false;
  } catch(e) {
    document.getElementById('result-text').textContent = '오류: ' + e.message;
  }
...
async function saveToNotion() {
  ...
  if (notionToken) {
    ...
    status.textContent = '설정 탭에서 Notion Token을 먼저 입력해주세요.';
  }
